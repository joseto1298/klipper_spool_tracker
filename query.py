#!/usr/bin/env python3
"""
Ejemplo: python query.py [db_path] [--job <id>] [--spool <id>]
...
def query_tracker(job_id: str = None, spool_id: int = None):
    cfg = load_config()
    http = cfg.get("http", {})
    host = http.get("host", "localhost")
    port = http.get("port", 8200)
    if http.get("enabled", True) is False:
        print("Error: servidor HTTP deshabilitado en config.json")
        return
    ...
    try:
        ...
        print(f"Error consultando tracker: {exc}")
        return
    ...
    for r in rows:
        print(f"Job {r['job_id']:>6} | Bobina {r['spool_id']:>3} | {r['filament_mm']:>8.2f} mm")
...
def query_local(db_path: str, job_id: str = None, spool_id: int = None):
    ...
    if job_id and spool_id:
        ...
        rows = cur.fetchall()
        for r in rows:
            print(f"Job {r[1]:>6} | Bobina {r[2]:>3} | {r[3]:>8.2f} mm")
    elif job_id:
        cur.execute(
        ...
        rows = cur.fetchall()
        for r in rows:
            print(f"Job {r[0]:>6} | Bobina {r[1]:>3} | {r[2]:>8.2f} mm")
...
if __name__ == "__main__":
    ...
    db = args[0] if args and args[0][0] != "-" else "spool_usage.db"
